...
def outputGenes(plot_data, ordered_in_sets,title,resultdir,key='Genes'):
    OUT=open('{}/orderedIntersectingGenes_{}'.format(resultdir,title),'w')
    for i in range(0,len(ordered_in_sets)):
        ordered_in_set=ordered_in_sets[i]  
        genes=list(plot_data.inters_df_dict[ordered_in_set][key])
        genes.insert(0,'_'.join(ordered_in_set))
        OUT.write(','.join(genes)+'\n')
    OUT.close()
    
def plot(data_dict, cols, title,resultdir,*, unique_keys=None, sort_by='size', inters_size_bounds=(1, np.inf),
    ...
    plot_data = DataExtractor(data_dict, all_columns,cols)
    ...
    ### Always put None at the Beginning ###
    inds=np.array([sum(['None' in ref for ref in grp]) for grp in ordered_in_sets]).astype(bool)
    notinds=np.invert(inds)
    ordered_in_sets=np.concatenate([ordered_in_sets[inds],ordered_in_sets[notinds]])
    ordered_out_sets=np.concatenate([ordered_out_sets[inds],ordered_out_sets[notinds]])
    ordered_inters_sizes=np.concatenate([ordered_inters_sizes[inds],ordered_inters_sizes[notinds]])
    ###
    
    ordered_dfs, ordered_df_names = plot_data.ordered_dfs, plot_data.ordered_df_names
    
    outputGenes(plot_data, ordered_in_sets,title,resultdir)
...
class UpSetPlot():
    ...
    def _prepare_figure(self, additional_plots):
        ...
        ax_setsize = plt.subplot(gs_top[-1:-setsize_h, setsize_w]) 
    ...
    def _base_sets_plot(self, sorted_sets, sorted_set_names):
        ...
        ax.set_xlim(0,50)#xlim[0] + gap, xlim[1] - gap)
        xlim = ax.get_xlim()
        ...
        ax.set_xlabel("Total Intersecting Genes\nwith MeTeOR", fontweight='bold', fontsize=FONTSIZE)#13)
    ...
    def _inters_sizes_plot(self, ordered_in_sets, inters_sizes):
        ...
        ax.set_ylabel("# of Genes Intersecting Between\nMeTeOR and Reference(s)", labelpad=6, fontweight='bold', fontsize=FONTSIZE)#13)
    ...
    def additional_plot(self, ax_index, kind, data_values, graph_args, *, labels=None):
        ...
        for l, text in labels.items():
            getattr(ax, 'set_%slabel' % l)(text, labelpad=3,
                                           fontweight='bold', fontsize=FONTSIZE) if l in ['x', 'y'] else None
        return ax


class DataExtractor:
    def __init__(self, data_dict, unique_keys,cols):
        """
    ...
    def extract_base_sets_data(self, data_dict, unique_keys,cols):
        """
        ...
        ### Choose order as Col order
        base_sets_order = np.argsort([cols.index(df_names[i]) for i in range(0,len(dfs))])#[::-1]
        ###
